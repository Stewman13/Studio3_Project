...
public class ExampleScene : MonoBehaviour {
	...
	void Update () {
		if (m_goSphere.GetComponent<ModifyGravity> ().TabletBuild == true) {
			m_fSpeed *= 0.95f;
			m_goSphere.GetComponent<Rigidbody> ().AddForce (m_vRotationDirection * Time.deltaTime * m_fSpeed / 2, ForceMode.Force);
	...
	private void MyCallbackMethod( SwipeControl.SWIPE_DIRECTION iDirection ) {
		...
		case SwipeControl.SWIPE_DIRECTION.SD_TOUCH:
			m_goSphere.GetComponent<ModifyGravity> ().turnGravOn = !m_goSphere.GetComponent<ModifyGravity> ().turnGravOn;
